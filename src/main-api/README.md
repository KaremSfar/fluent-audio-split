...
`YouTubeAudioImport__ImpersonateClient`, `YouTubeAudioImport__ProxyUrl`,
`YouTubeAudioImport__FfmpegPath`,
...
- The API host must have outbound HTTPS access to YouTube and its media/CDN hosts. Datacenter/VPS IPs are commonly challenged or throttled, so set `YouTubeAudioImport__ProxyUrl` to a residential/trusted egress proxy (e.g. `http://<host>:3128`) to route extraction and media download through it. Leave it empty to connect directly. In Docker, set `YOUTUBE_IMPORT_PROXY_URL` in the root `.env`, which Compose maps to this setting.
- On a datacenter IP, YouTube may return a “Sign in to confirm you're not a bot” challenge at extraction time or time out the media download from its CDN after issuing a signed URL. A residential egress via `YouTubeAudioImport__ProxyUrl` resolves both cases. Keep request volume low and use only media you are authorized to use.
